...
Vd, Vc, Vth, dVth,Ath, cad =volume (CR)
...
fig, ax = plt.subplots()
plt.bar(x, IMEP,width=0.3,color=['C0','C1', 'C2', 'C4'])
...
#from jauda import power #call function to calculate volume
#Pi_net, Pi_gross, Ti_gross, Ti_net =power (CR)

n=2 #apgriezieni ciklā
Wci_net=np.trapz(P_Test2*10**(-5)*dVth)

Ti_net=(Wci_net)/n*2*np.pi
     
#Pi_gross=Wci_gross*cad/n
Pi_net=Wci_net*cad/n
